...
将上一步骤获取到的 Vid、RefreshToken 和 DeviceId 保存到 JSON 格式的配置文件`account.json`中：
...
下载后解压到任意目录，将配置文件 `account.json` 放在与可执行文件同一目录下，然后运行：
...
将配置文件 `account.json` 放在 `app.cs` 所在目录下，然后运行：

刷新Token：

```bash
dotnet run app.cs -- refresh
```
...
启用[签到工作流](https://github.com/chieeh/WereadCheckin/blob/main/.github/workflows/checkin.yml)
